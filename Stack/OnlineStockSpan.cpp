...
public:
    ...
    int next(int price) {
        ...
        while(!decreasing_stack.empty() && price >= decreasing_stack.top().first) {
            // current element is greater than top.
            // Need to remove element from the stack
            span += decreasing_stack.top().second;
            decreasing_stack.pop();
        }

        
        decreasing_stack.push({price, span});
        return span;
    }
private:
...
int main() {
    ...
    for (int i = 0; i < prices.size(); i++)
        cout << "Output: " << s.next(prices[i]) << endl;
    return 0;
}